...
// class CargadorActualizacion_TDCCooperativaJEP extends CargaModelo_Actualizaciones_Abstract implements CargaModelo_Actualizaciones_Interface, Iterator{
class CargadorActualizacion_TDCCooperativaJEP extends CargaModelo_Uploadable_Abstract implements CargaModelo_Uploadable_Interface, Iterator{
	private $header;
	private $data;
	...
	function processRecord(&$line) {
		...
		// if ($line['cliente']['Numero Operacion']!='80300002377') return null;
		// print_arr($line);
		...
		// 	$cuenta->add_actualizacion('PAGO',$pago*-1,$fecha_pago,$p['Hora de cobro']);
		// }
...
class ActualizacionTDC_CooperativaJEP extends CargaModelo_Handler_Abstract {
	...
	function execute($step, &$__data) {
		...
		switch($step) {
			case '3':
				$uploader = new CargadorActualizacion_TDCCooperativaJEP($SM->carga_process['source_file']);
				return $uploader;
			break;
			
			case '2':
				error_reporting(E_ALL);
				$dir = _BASE_USER_PATH.'/CooperativaJEP_TDC/Cargas/actualizacion/';
			...
			case '1':
				...
				<form method="POST" action="?'.Helpers::arr_to_url($_GET,array(),array('step2'=>'2','token'=>$SM->carga_process['expected_token'])).'" enctype="multipart/form-data">
					...
					foreach ($archivos_permitidos as $file => $campos){
						$__data['_T']['maincontent'].='<img src="/cobranzas/user/CooperativaJEP_TDC/Uploaders/'.(in_array($file,array_keys($error_file))?'error.png':'ok.png').'" width="21" height="21">  '.$file.'<br>';
