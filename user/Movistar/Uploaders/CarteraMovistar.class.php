...
class CarteraMovistarExcel extends CargaModelo_Uploadable_Abstract implements CargaModelo_Uploadable_Interface, Iterator{
	private $spreadsheet;
	private $sheet;
	private $lastRow;
	private $lastColStr;
	...
	function processRecord(&$line) {
		try {
			...
			if (trim($line['cobro empresa1'][1])=='')
				throw new Exception('cobro empresa1 sin valor en línea '.$this->ptr.' ('.$line['cobro empresa1'][1].')');
			if (!is_numeric(trim($line['cobro empresa1'][1])))
				throw new Exception('cobro empresa1 inválido en línea '.$this->ptr.' ('.$line['cobro empresa1'][1].')');
			if (trim($line['ddias_vencimiento'][1])=='' or !in_array(trim($line['ddias_vencimiento'][1]),array('0','30','60','90')))
				throw new Exception('ddias_vencimiento inválido debe ser (0,30,60 ó 90) en línea '.$this->ptr.' ('.$line['ddias_vencimiento'][1].')');
			...
			$cuenta->valor_actual=floatval($line['cobro empresa1'][1]);

			$cuenta->add_actualizacion('PAGO',(floatval($line['pagos'][1])>0?floatval($line['pagos'][1])*(-1):floatval($line['pagos'][1])),date('Y-m-d'));
			
			$ajuste = floatval($line['ajustes'][1]);
			$cuenta->add_actualizacion(($ajuste > 0 ?'AJUSTE+':'AJUSTE-'),$ajuste,date('Y-m-d'));

			$factura_mes = floatval($line['factura del mes'][1]);
			$cuenta->add_actualizacion(($factura_mes > 0 ?'AJUSTE+':'AJUSTE-'),$factura_mes,date('Y-m-d'));
			...
			$ret['otros_datos']['cobro empresa1'] = $line['cobro empresa1'][1];
			...
			unset($line['cobro empresa1']);
			unset($line['cobro empresa2']);
			...
			echo (floatval($line['ajustes'][1])<0?'AJUSTE+':'AJUSTE-');
			print_arr($e);
			print_arr($line);
			die();
		}
...
class CarteraMovistar extends CargaModelo_Handler_Abstract {
	function getTipoBase() {
		return 'Movistar - Carga de Cartera Nueva';
	}
	...
	function execute($step, &$__data) {
		...
		switch($step) {
			case '3':
				$uploader=new CarteraMovistarExcel(_TMP_UPLOAD_FOLDER.'/'.$SM->carga_process['source_file']);
				return $uploader;
			...
			case '1':
				$__data['_T']['maincontent']='<h1>Carga de Cartera Nueva - Movistar</h1>
