<?php
class MejorGestion implements Reporte_Interface {
	private $cache=array();
	...
	public function execute($_post,$_get,&$result,&$additional_data=array(),&$_T=array()) {
		//if($_SERVER['REMOTE_ADDR']!='192.168.29.188') die('Personalizando reporte');
		$db=DB::getInstance();
		...
		switch($_get['step']) {
			...
			default:
				...
				$_T['maintitle']='INTERAGUA - Reporte de Mejor Gestion';
				$_T['maincontent']='
				<script>
