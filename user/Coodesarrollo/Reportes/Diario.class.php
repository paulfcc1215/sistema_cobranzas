<?php
class Diario implements Reporte_Interface {
	private $cache=array();
	...
	public function execute($_post,$_get,&$result,&$additional_data=array(),&$_T=array()) {
		...
		switch($_get['step']) {
			case '2':
                ...
                if(!preg_match('#^\d{2}/\d{2}/\d{4}#',$_post['dia'])) throw new Exception('Fecha invalida');

				$dia=Helpers::dmy2ymd($_post['dia']);
				$proceso=getProceso($_post['id_proceso']);
				if(!$proceso) throw new Exception('Proceso no existe');

				$reporte = 'diario_'.strtolower($udn['udn']).'_'.date('dmY').'.txt';
				//Marco pide que se quite el encabezado
				...
				$q='
					...
					WHERE
						...
						AND date(g.fecha_inicio) BETWEEN \''.$dia.'\' AND \''.$dia.'\'
			...
			default:
				...
				$_T['maintitle']='Banco Desarrollo - Reporte diario';
				$_T['maincontent']='
				<script>
				...
				<form method="POST" action="?'.Helpers::arr_to_url($_get,array(),array('step'=>'2')).'">
					...
					<br><br>
					<b>Indique el día:</b><br>
					<input type="text" name="dia" class="fecha" value="'.date('d/m/Y').'">
					<br><br>
