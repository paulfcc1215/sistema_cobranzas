<?php
class MejorGestion_por_fechas_ban_ecuador implements Reporte_Interface {
	private $cache=array();
		...
		switch($_get['step']) {
			case '2':
				...
				foreach($gestiones_x_cuenta as $num_cuenta=>$gestiones) {
					...
					if ($tipificacion['_metadata']['es_contacto_primera_persona'] || $tipificacion['_metadata']['es_contacto_tercero']){
						$contacto = 'CONTACTADO';
					}
					$line = array(
						//'identificacion_deudor',
			...
			default:
				...
				$_T['maintitle']='BanEcuador - Mejor gestión por fechas';
				$_T['maincontent']='
				<script>
