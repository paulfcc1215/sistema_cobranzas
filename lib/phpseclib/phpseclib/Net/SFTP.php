...
class SFTP extends SSH2
{
    ...
    */
     ...
     */
    function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null, $max_bandwidth_KiBps=0)
    {
        if (!($this->bitmap & SSH2::MASK_LOGIN)) {
            return false;
        }
        ...
        // make the calculation to limit the bandwidth
        if($max_bandwidth_KiBps>0) {
            $uwait=4070*1000/($max_bandwidth_KiBps*1024);
        }else{
            $uwait=0;
        }
        while ($dataCallback || ($size === 0 || $sent < $size)) {
            ...
            //echo 'Sent: '.$sent.' + '.$sftp_packet_size."\n";
            if($uwait > 0) {
                usleep($uwait*1000);
            }
        }
