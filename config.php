...
try {
    ...
    define ('_SHOW_DISABLED_PROCESSES_IN_SEARCH',true);
    ...
    define('_DB_PASSWORD','postgres');  //orangeDragon$2017 desde otro host
    define('_DB_DBNAME','cobranzas');
    

    // schema de tablas log
    define('_DB_SCHEMA_LOG_TABLES','log');
    // duracion maxima de archivos temporales (default: 1 hora)
    define('_TMP_MAX_TIME',3*60*60);
    ...
    try{
        $repo_info_conection = array(
            'host'=>'192.168.180.216',
            'user'=>'postgres',
            'password'=>'postgres',
            'dbname'=>'repositorio'
        );
        DB::connect('pgsql',$repo_info_conection,'repo');
    }catch(Exception $ex){
        Log::addLog('REPO_CONECTION',__FILE__,$repo_info_conection);
        $mail = new Helpers_Mail();
        $to = array(
            'paul.cedeno@recappt.com',
            'joseph.espinoza@recappt.com',
            'jefferson.garrido@soportegrupocant.com',
            'diego.gaybor@recapt.com.ec',
        );
        $mail->add_attachment($file_zip);
        $subject = 'NOTIFICACION AUTOMATICA - ORION PERDIO CONEXION CON REPOSITORIO';
        $content = 'Estimados<br><br><br>No se ha logrado conexión entre repositorio y Orión';
    ...
    $_T=array(
        'basepath'=>'template/',
        'basepath_metronic'=>'cobranzas/template/metronic/dist/',
        'title'=>'Orión Sistema Integral de Cobranzas',
        'css_files'=>array()
    );
...
function debug() {
    if(in_array($_SERVER['REMOTE_ADDR'],array('192.168.29.190'))) {
        return true;
    }
    return false;
}