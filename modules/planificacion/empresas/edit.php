...
switch($_GET['step']) {
    case '2':
        try {
            ...
            <hr>
            <a href="?mod=planificacion/empresas/index">Regresar</a>
            ';
        }catch(Exception $e) {
            $db->rollback();
            $error=$e->getMessage();
    ...
    default:
		...
		$_T['maincontent'].='
		  ...
		  <button class="btn btn-danger" type="button" onclick="window.location=\'?mod=planificacion/empresas/index\'">Cancelar</button> 
		  <button class="btn btn-warning" type="button" onclick="window.location=\'?mod=planificacion/empresas/del&id='.$_GET['id'].'\'">Eliminar</button> 
