...
switch($_GET['step']) {
    case '2':
        try {
            ...
            <button class="btn btn-primary" type="button" onclick="window.location=\'?'.Helpers::arr_to_url($_GET,array('step'),array('mod'=>'planificacion/empresas/index')).'\'">Regresar</button>
            ';
        }catch(Exception $e) {
            $error=$e->getMessage();
            goto lbl_default;
    ...
    default:
        ...
        <button class="btn btn-primary" type="button" onclick="window.location=\'?'.Helpers::arr_to_url($_GET,array(),array('mod'=>'planificacion/empresas/edit','id'=>$empresa->id_empresa)).'\'">Cancelar</button>
        </form>
        ';
    break;
}