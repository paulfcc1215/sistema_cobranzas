<?php
	...
	if(!Auth::hasPrivileges('AUTH_PARAMETROS_SCRIPTS_MODIFICAR')) throw new Exception('No autorizado - AUTH_PARAMETROS_SCRIPTS_MODIFICAR');

	$db = DB::getInstance();
	$ams = AutoModel::getInstance('campanas','scripts',$db);
	$script = $ams->getById($_GET['id']);

	$_T['maintitle']='Parámetros del sistema - Scripts - Modificar Script';
	...
	try {
		...
		$script->id_proceso=$_POST['proceso'];
		$script->descripcion=$_POST['descripcion'];
		$script->script=$_POST['contenido'];

		$_T['maintitle']='Gestión de Scripts - Nuevo Script';
		$_T['maincontent'].='<a href="?mod=scripts/index">Regresar</a><br><br>';
		$_T['maincontent'].='
			<div class="alert alert-success" role="alert">Script registrado correctamente!</div>
			<hr>
			<a href="?mod=scripts/index">Regresar a Scripts</a>';

	}catch(Exception $e) {

		//get udn y campana del script
		$q = 'SELECT u.id_udn,c.id_campana 
		FROM campanas.proceso p
		JOIN campanas.campana c USING (id_campana)
		JOIN estructura.udn u USING (id_udn)
		WHERE id_proceso='.$script->id_proceso;
		$q0= $db->query($q);
		$aux = $db->fetchOne($q0);
					...
					<td>
						<table class="table_form">
							<tr>
								<td>
									<label>UDN:</label>
									<select id="udn" class="form-control" onchange="change_udn()">'.UI_Helper::array_to_options($udns,($_POST['udn']==''?$aux['id_udn']:$_POST['udn']),true).'</select>
								</td>
								<td>
									<label>Campaña:</label>
							...
							<tr>
								<td colspan="3">
									...
									<input name="descripcion" value="'.($_POST['descripcion']==""?$script->descripcion:$_POST['descripcion']).'" placeholder="Descripción" class="form-control"></input>
								</td>
							</tr>
							<tr>
								<td colspan="3">
									<label>Script:</label>
									<textarea class="ckeditor" name="contenido">'.($_POST['contenido']==''?$script->script:$_POST['contenido']).'</textarea><br>
								</td>
							</tr>
						</table>
					</td>
